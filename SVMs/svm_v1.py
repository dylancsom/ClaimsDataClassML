...
Corpus = pd.read_excel(r"training_dataset_excel_path")
...
for index, entry in enumerate(Corpus['text']):
    ...
    for word, tag in pos_tag(entry):
        if word not in stopwords.words('english') or word in ["specific target stopwords for exclusion"]:
            if word.isalpha() and word not in ["total dataset stopwards for exclusion"]:  # Check if word is not "insured" or "insured's"
...
joblib.dump(SVM, 'saved_trained_model.joblib')
