...
loaded_model = joblib.load('saved_trained_model_path.joblib')
loaded_vectorizer = joblib.load('vecterizer_path.joblib')
...
def remove_special_characters(text):
    ...
    return text
# Load the new data
new_data = pd.read_excel(r"test_dataset_excel_path")
...
for index, entry in enumerate(new_data['LossDescription']):
    ...
    for word, tag in pos_tag(entry):
        if word not in stopwords.words('english') or word in ["stopwords (same as trained model version)"]:
            if word.isalpha():
...
output_path = 'processed_test_data_path'
